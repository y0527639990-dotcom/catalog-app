    ...
    <div className="rounded-2xl bg-white p-6 shadow-sm">
      ...
      {isSuperAdmin ? (
        <p className="mt-2 text-sm text-gray-600">
          מנהל ראשי — ניתן לערוך שם חנות, שם משתמש ולאפס סיסמה.
        </p>
      ) : (
        <p className="mt-2 text-sm text-gray-600">
          רשימת חנויות בלבד. לניהול סיסמאות — התחבר כמנהל ראשי.
        </p>
      )}
          ...
          <table className="min-w-full text-right text-sm">
            <thead>
              <tr className="border-b border-gray-200 text-gray-500">
                ...
                {isSuperAdmin && <th className="px-3 py-2">ניהול</th>}
              </tr>
            </thead>
            <tbody>
              {stores.map((store) => (
                <tr key={store.id} className="border-b border-gray-100">
                      {editId === store.id && isSuperAdmin ? (
